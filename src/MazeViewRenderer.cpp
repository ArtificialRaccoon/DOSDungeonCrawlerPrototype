...
void MazeViewRenderer::RenderVisionCone(Dungeon &currentDungeon, VisionCone &wallCone, VisionCone &decoCone)
{
    ...
    for(int i = 0; i < 7; i++)
    {
        if(!(i >= 2 && i < 5 && wallCone.Tier3[2] > 0))
        {
            if(!(i == 3 && wallCone.Tier2[3] > 0) && !(i == 3 && wallCone.Tier1[3] > 0))
            {
    ...
    for(int i = 1; i < 6; i++)
    {
        if(!(i == 3 && wallCone.Tier3[2] > 0) && !(i == 3 && wallCone.Tier2[3] > 0))
        {
    ...
    //Render SideB
    if(wallCone.Tier3[1] > 0 && wallCone.Tier3[2] == 0)
        DrawSideWall(currentDungeon, SIDEB, false, wallCone.Tier3[1], SIDEB_STARTX);
    if(wallCone.Tier3[3] > 0 && wallCone.Tier3[2] == 0) 
